...
- Override counter entries (keys beginning with `ov_s:`, `ov_c:`, or `ov_b:`) whose value is not an integer in the range 0–4294967295 MUST be discarded as a group: discard all `ov_s:`, `ov_c:`, and `ov_b:` entries for the affected context ID; do not discard the corresponding frontier entry.
...
**Mark-unread:** increment S to `max(S, C) + 1`; set B to the current effective frontier value for the context. C is unchanged.

**Mark-read (explicit):** advance the frontier to cover the context as normal; increment C to `max(S, C) + 1`. S and B are unchanged.
...
- **Byte cost:** channel UUID context (36 chars) — live override ~138 bytes; tombstone ~45 bytes. Both fit comfortably within the existing 32 KiB plaintext budget.
- **10,000-key limit:** override entries count toward the existing per-blob validation limit. Clients SHOULD compact dead overrides aggressively and MAY enforce an active-override cap.
...
The design was verified by bounded exhaustive model checking prior to this amendment. See `docs/formal/nip-rs-unread/` for the full model (`model.py`, `exhaustive.py`), 9-mutant harness (`mutation.py`), and design notes (`NOTE.md`). The harness confirms all four load-bearing protocol requirements — clear-wins tie policy, tombstone floor, mandatory canonical publication, and atomic slot-grouping with unescape-before-group — are necessary: each mutant that drops one of these rules produces a detectable witness of permanent false-clear or resurrection.
